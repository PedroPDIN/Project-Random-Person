...
export default function InfoUser({ picture, name, gender, age, phone, cell, email }: Props) {
  return (
    <div className="flex max-micro-screen:flex-col justify-around items-center bg-white border-solid border-2 border-[rgba(0,0,0,.25)] rounded-md shadow-lg px-4 py-12 dark:bg-[#2c2e31] transition ease-out">
      <div className="flex flex-col justify-center max-micro-screen:items-center">
        <Image
          src={ picture }
          alt={`${name} profile`}
          width={190}
          height={190}
          className="rounded-full p-1 border-solid border-2 border-[rgba(0,0,0,.25)] z-10 bg-white max-micro-screen:w-[120px] max-micro-screen:h-[120px]"
        />

        <p className="flex flex-col text-center font-bold text-xl max-micro-screen:text-[18px]">
          <span className="text-[#999]">Olá, meu nome é</span>
          {name}
        </p>
      </div>

      <div className="border-separate border-2 border-[rgba(0,0,0,.25)] h-[200px] rounded-full dark:border-white max-micro-screen:hidden"></div>

      <div className="grid gap-3 max-micro-screen:w-[90%] max-micro-screen:text-[14px]">
